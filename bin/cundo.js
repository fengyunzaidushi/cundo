...
program
  .name("cundo")
  .description(
    "Enhanced undo tool for Claude Code sessions - Fixed and improved version"
  )
  .version("1.0.0");
  ...
  .action(async (options) => {
    try {
      ...
      if (!options.local) {
        ...
        // Use local cundo tracking
        const sessionId =
          options.session || (await SessionTracker.getCurrentSession());
        if (!sessionId) {
          console.log(chalk.yellow("No local cundo session found."));
          return;
        }
...
program
  ...
  .option("--local", "Use local cundo tracking instead of Claude sessions")
    ...
    try {
      ...
      if (!operationId) {
        const choices = operations.map((op, index) => {
          ...
          if (operationsToUndo > 1) {
            name += chalk.red(
              ` (+ ${operationsToUndo - 1} more will be undone)`
            );
          }
      ...
      if (!options.yes) {
        ...
        for (let i = 0; i < operationsToUndo.length; i++) {
          ...
          console.log(
            `${chalk.bold(`${i + 1}.`)} ${chalk.cyan(
              op.type
            )} - ${formatDistance(op.timestamp)}`
          );
      ...
      console.log(
        chalk.bold(
          `\nCompleted: ${chalk.green(successCount)} successful, ${chalk.red(
            failCount
          )} failed`
        )
      );
  ...
  .action(async (operationId, options) => {
    try {
      ...
      if (options.local) {
        ...
        console.log(
          chalk.yellow("Redo for local tracking is not yet implemented.")
        );
        return;
      } else {
        // Use Claude Code sessions
            ...
            if (entry.type === "assistant" && entry.message?.content) {
              for (const content of entry.message.content) {
                if (content.type === "tool_use") {
                  const operation = parser2.extractOperation(
                    content,
                    entry.timestamp
                  );
                  if (operation) {
        ...
        operations = await undoTracker.getUndoneOperationsList(
          originalOperations,
          sessionFile
        );
      }
      ...
      if (!operationId) {
        const choices = operations.map((op, index) => {
          ...
          if (operationsToRedo > 1) {
            name += chalk.green(
              ` (+ ${operationsToRedo - 1} more will be redone)`
            );
          }
      ...
      if (!options.yes) {
        console.log(
          chalk.yellow(
            `\n${i18n.t("header.this_will_redo", {
              count: operationsToRedo.length,
            })}\n`
          )
        );
        ...
        const confirm = await inquirer.prompt([
          {
            type: "confirm",
            name: "proceed",
            message: i18n.t("prompt.confirm_redo", {
              count: operationsToRedo.length,
            }),
            default: false,
          },
    ...
    try {
      ...
      if (!operationId) {
        const choices = operations.map((op, index) => {
          ...
          if (operationsToUndo > 1) {
            name += chalk.gray(
              ` (+ ${operationsToUndo - 1} more would be undone)`
            );
          }
      ...
      for (let i = 0; i < operationsToUndo.length; i++) {
        const op = operationsToUndo[i];
        console.log(
          `${chalk.bold(`${i + 1}.`)} ${chalk.cyan(op.type)} - ${formatDistance(
            op.timestamp
          )}`
        );
      ...
      console.log(
        chalk.gray("💡 To actually perform these undos, run: cundo undo")
      );
    } catch (error) {
      console.error(chalk.red(`Error: ${error.message}`));
    }
...
program
  ...
  .option("--local", "Show local cundo sessions instead of Claude sessions")
  .action(async (options) => {
    try {
      if (options.local) {
  ...
  .action(async (lang) => {
    try {
      if (!lang) {
        ...
        console.log(
          chalk.bold(
            `\nCurrent language: ${chalk.cyan(current.name)} (${
              current.code
            })\n`
          )
        );
      ...
      console.log(
        chalk.green(i18n.t("msg.language_set", { language: newLang.name }))
      );
    } catch (error) {
      const available = i18n
        .getAvailableLanguages()
        .map((l) => l.code)
        .join(", ");
      console.error(
        chalk.red(i18n.t("msg.language_invalid", { languages: available }))
      );
    }
  });
